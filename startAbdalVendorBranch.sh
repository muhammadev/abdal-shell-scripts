...
function ctrl_c() {
  {
    git restore "C:\laragon\www\abdal\webpack.mix.js" && echo -e "\n >>> switch webpack.mix.js file to original \n"
  } || {
    echo -e "\n >>> couldn't resotre webpack.mix.js! \n"
  }
}
trap ctrl_c INT
...
if [ $1 ] && [[ $(git branch --list "$1") ]]; then
  ...
  if [[ $p = y ]]; then
    ...
    {
      git checkout dev/vendor-front &&
        git pull origin dev/vendor-front &&
        git merge dev/vendor &&
        git checkout $1 &&
        git merge dev/vendor-front &&
